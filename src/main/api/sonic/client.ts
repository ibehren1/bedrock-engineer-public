...
const sonicLogger = createCategoryLogger('sonic:client')
...
export class NovaSonicBidirectionalStreamClient {
  ...
  public async initiateSession(sessionId: string): Promise<void> {
    ...
    try {
      ...
      // Log the full error to the winston log. This is critical for diagnosing Nova Sonic
      // failures: SDK deserialization errors hide the raw service response on the
      // non-enumerable `$response`/`$responseBodyText` fields, which console.error drops
      // and DevTools truncates.
      const anyErr = error as any
      sonicLogger.error(`Error initiating Nova Sonic session ${sessionId}`, {
        name: error instanceof Error ? error.name : typeof error,
        message: error instanceof Error ? error.message : String(error),
        metadata: anyErr?.$metadata,
        responseBodyText: anyErr?.$responseBodyText,
        responseStatus: anyErr?.$response?.statusCode,
        stack: error instanceof Error ? error.stack : undefined
      })
      console.error(`Error in session ${sessionId}: `, error)
      // Raw Error objects don't survive Socket.IO's JSON serialization (name/message
      // are non-enumerable and serialize to {}). Extract the useful fields explicitly
      // so the renderer can surface the real cause instead of a generic message.
  ...
  private async processResponseStream(sessionId: string, response: any): Promise<void> {
    ...
    try {
      ...
      // Capture the raw service response hidden on the SDK error, which is where Nova
      // Sonic eventstream deserialization failures surface their real cause.
      const anyErr = error as any
      sonicLogger.error(`Error processing Nova Sonic response stream ${sessionId}`, {
        name: error instanceof Error ? error.name : typeof error,
        message: error instanceof Error ? error.message : String(error),
        metadata: anyErr?.$metadata,
        responseBodyText: anyErr?.$responseBodyText,
      ...
      this.dispatchEvent(sessionId, 'error', {
        ...
        message: error instanceof Error ? error.message : 'Error processing response stream',
        details: error instanceof Error ? error.message : String(error)
      })
    }
  }
