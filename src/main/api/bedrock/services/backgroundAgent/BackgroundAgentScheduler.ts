...
/**
 * Resolve the system's local IANA timezone (e.g. "America/New_York"),
 * falling back to UTC if it cannot be determined.
 */
...
export class BackgroundAgentScheduler {
  ...
  constructor(context: ServiceContext) {
    ...
    // Cronジョブに使用するタイムゾーンを解決（システムのローカルタイムゾーン）
    this.timezone = getSystemTimezone()
    logger.info('Scheduler timezone resolved', { timezone: this.timezone })
  ...
  private startCronJob(task: ScheduledTask): void {
    try {
      ...
      const cronJob = cron.schedule(
        ...
        {
          timezone: this.timezone // システムのローカルタイムゾーンを使用
        }
      )
   ...
   * 次回実行時刻を計算
   *
   * node-cronの公開APIを使い、cron式とスケジューラのタイムゾーンに基づく
   * 実際の次回実行時刻を求める。計算用に一時的なタスクを生成し、次回時刻を
   * 取得した後は破棄してリソースリークを防ぐ。
   */
  private calculateNextRun(cronExpression: string): number | undefined {
    if (!cron.validate(cronExpression)) {
      logger.warn('Cannot calculate next run for invalid cron expression', {
        cronExpression
      })
      return undefined
    }

    // 計算専用の一時タスク（実際のジョブは登録しない）
    let tempTask: cron.ScheduledTask | undefined
    try {
      tempTask = cron.createTask(cronExpression, () => {}, {
        timezone: this.timezone
      })
      // getNextRun() は開始状態でのみ次回時刻を返すため、一時的に開始する
      tempTask.start()
