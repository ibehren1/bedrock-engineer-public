...
export class ConverseService {
  ...
  private async prepareCommandParameters(props: CallConverseAPIProps): Promise<{
    ...
    if (isFable5) {
      ...
      converseLogger.debug('Enabling Thinking Mode (always-on for Fable 5)', {
        modelId,
    ...
    if (
      ...
      // Determine the correct thinking type to send based on model capabilities
      const supportedTypes = getSupportedThinkingTypes(modelId)
