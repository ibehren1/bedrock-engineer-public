...
export class ChatSessionManager {
  ...
  deleteSessions(sessionIds: string[]): void {
    ...
    // recentSessions からまとめて除外
    const recentSessions = this.metadataStore.get('recentSessions')
    this.metadataStore.set(
      'recentSessions',
      recentSessions.filter((id) => !idSet.has(id))
    )

    // 削除対象にアクティブセッションが含まれる場合はクリア
    const activeSessionId = this.metadataStore.get('activeSessionId')
