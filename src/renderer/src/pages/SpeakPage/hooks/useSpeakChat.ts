...
export function useSpeakChat(
  ...
  const socketEvents: Partial<SocketEvents> = {
    ...
    error: (error) => {
      ...
      // Set detailed error state based on the error content. Server-emitted errors are
      // plain objects with message/details; fall back through the available fields so we
      // surface the real cause rather than a generic "Unknown error".
      const errorMessage =
        typeof error === 'string'
          ? error
          : error?.message || error?.details || error?.name || 'Unknown error'
      setErrorState({
        type: 'connection',
  ...
  useEffect(() => {
    switch (socket.status) {
      ...
      case 'error':
        setStatus('error')
        // The socket `error` event handler above may have already captured a detailed
        // message (e.g. the real AWS error from session init). Don't clobber it with the
        // generic fallback — only use the generic message when nothing specific is set.
        setErrorState((prev) =>
          prev
            ? prev
            : {
