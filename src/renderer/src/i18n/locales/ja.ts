...
const HomePage = {
  ...
  'Welcome to Bedrock Engineer': '{{appName}} にようこそ',
...
const SettingPage = {
  ...
  'Complete permissions for all Bedrock Engineer features including translation and video generation':
    '翻訳と動画生成を含む、{{appName}} の全機能に対する完全な権限',
...
const SpeakPage = {
  ...
  'voiceChat.error.contentFilter':
    'Voice ChatがAmazon Bedrockのコンテンツフィルターによってブロックされました。通常、選択中のエージェントのシステムプロンプトの表現が原因です。システムプロンプトを編集するか、別のエージェントに切り替えてください。',
...
const TodoModal = {
  'Show chat history': 'チャット履歴を表示',
  'Hide chat history': 'チャット履歴を非表示',
