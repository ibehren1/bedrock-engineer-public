...
export interface SettingsContextType {
  ...
  setSidebarHiddenItems: (items: string[]) => void
  ...
  availableModels: LLM[]
  /** Models filtered by the user's visible-models allowlist (falls back to all models when the allowlist is empty). */
  visibleModels: LLM[]
  llmError: any
...
export const SettingsProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // Sidebar Settings
  const [sidebarHiddenItems, setStateSidebarHiddenItems] = useState<string[]>([])
  ...
  // Models to expose in the chat model selector, filtered by the user's allowlist.
  // An empty allowlist means "show all". The currently selected model is always
  // kept visible so it can't disappear from the picker after being deselected.
  const visibleModels = useMemo<LLM[]>(() => {
    const allowlist = bedrockSettings.visibleModelIds
    if (!allowlist || allowlist.length === 0) {
      return availableModels
    }
    return availableModels.filter(
      (model) => allowlist.includes(model.modelId) || model.modelId === currentLLM?.modelId
    )
  }, [availableModels, bedrockSettings.visibleModelIds, currentLLM?.modelId])
  ...
  useEffect(() => {
    ...
    const sidebarHiddenItemsSetting = window.store.get('sidebarHiddenItems')
    if (sidebarHiddenItemsSetting !== undefined) {
      setStateSidebarHiddenItems(sidebarHiddenItemsSetting)
    }
    ...
    if (storedBedrockSettings) {
      setBedrockSettings({
        ...
        visibleModelIds: storedBedrockSettings.visibleModelIds || []
      })
    }
  ...
  const setSidebarHiddenItems = useCallback((items: string[]) => {
    setStateSidebarHiddenItems(items)
    window.store.set('sidebarHiddenItems', items)
  }, [])
  ...
  // User Avatar Settings function
  const setUserEmoji = useCallback((emoji: string) => {
    setStateUserEmoji(emoji)
